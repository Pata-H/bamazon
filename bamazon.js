...
var displayInventory = function(){
  connection.query("SELECT * FROM products", function(err,res){
    ...
    for (var i = 0; i < res.length; i++) {
      console.log("Item ID: " + res[i].item_id + "\n" + "Product Name: " + res[i].product_name + "\n" ,"Department Name: " ,res[i].department_name + "\n" ,"Price: " + res[i].price + "\n" + "Available Quantity: " + res[i].stock_quanity + "\n----------------------------");
    }
    ...
    for (var i=0;i<res.length;i++){
      if(res[i].item_id === chosenProductID){
        ...
        inquirer.prompt([productQuantityPrompt]).then(function(inquirerResponse){
          ...
          if ((res[id].stock_quanity - chosenQuantity) >= 0) {
            // subtract the inputted number from the product's stock_quanity and set as variable
            var newQuantity = res[id].stock_quanity - chosenQuantity;
            //total cost
            var totalCost = res[id].price * chosenQuantity;
            // update value in database with newQuantity
            var sql = "UPDATE ?? SET ?? = ? WHERE ?? = ?";
            var values = ['products', 'stock_quanity', newQuantity, 'item_id', chosenProductID];
          ...
          // if the number is greater than the product's stock_quanity amount, alert the user & restart the promptCustomer function
          else {
